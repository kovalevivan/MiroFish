...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Запрашивает одну страницу с повторами по экспоненциальной схеме для сетевых/IO-сбоев."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Постранично получает узлы графа, максимум `max_items` элементов; каждый запрос идет с повтором."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Постранично получает все ребра графа и возвращает полный список; каждый запрос идет с повтором."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
