<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">История запусков</span>
      <div class="section-line"></div>
    </div>
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- 卡片标题（使用Задача симуляции的前20字作为标题） -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- 卡片描述（Задача симуляции完整展示） -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>Связанные файлы отсутствуют</div>
              </div>
            </div>
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">Просмотр запуска</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">Аналитический отчет</span>
              </button>
            </div>
            <!-- 不可回放提示 -->
            <div class="modal-playback-hint">
              <span class="hint-text">Шаг 3 «Запуск симуляции» и шаг 5 «Глубокое взаимодействие» доступны только во время активного запуска. Историческое воспроизведение не поддерживается.</span>
            </div>
          </div>
        </div>
      </Transition>
...
// 从Задача симуляции生成标题（取前20字）
const getSimulationTitle = (requirement) => {
  if (!requirement) return 'Симуляция без названия'
...
// 导航到Аналитический отчет页面（Report）
const goToReport = () => {
...
const loadHistory = async () => {
  try {
    ...
    console.error('Не удалось загрузить историю проектов:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
...
onMounted(async () => {
  ...
  // Ожидание DOM 渲染后Инициализация观察器
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.status-icon:nth-child(3).available { color: #10B981; } /* Аналитический отчет - 绿色 */
...
/* 进度状态颜色 */
.card-progress.completed { color: #10B981; }    /* Готово - 绿色 */
.card-progress.in-progress { color: #F59E0B; }  /* В процессе - 橙色 */
